    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.getVercodeBtn2:
                ...
                } else if (isDigit1(phone2) == false){      //验证手机号内容是否都为数字
                    Toast.makeText(resetpwd.this, "手机号码不符合规范（它应该是纯数字）", Toast.LENGTH_LONG).show();
                }else if (phone2.length() != 11){
                    Toast.makeText(resetpwd.this, "手机号码不符合规范(它应该有11位)", Toast.LENGTH_LONG).show();
                }else{
                    String restring = sentmessage(phone2);
                    if(restring == null){
                        Toast.makeText(resetpwd.this, "连接服务器失败，该手机号码今日发送验证码超过五次", Toast.LENGTH_LONG).show();
                    }else {
            ...
            case R.id.resetBtn:
                re =null;       //获取返回值之前先将re置为空，以防误判
                setString2();
                if (pwd2.equals(confirm2) == true) {         //直接使用  pwd2 == confirm2  会出现  密码与确认密码不一致  的错误
                    Log.i(TAG, "pwd2.equals(confirm2) ==" + pwd2.equals(confirm2) );
                    String restring2 = checkvercode(phone2,vercode2);
                    Log.i(TAG, "restring2===" + restring2);
                    if(restring2 == "验证码正确"){
                        re =null;       //获取返回值之前先将re置为空，以防误判
                        ...
                        if (re == null) {
                            ...
                            intent2.setClass(resetpwd.this, Login.class);

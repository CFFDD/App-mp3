...
//                Log.i(TAG,"PausePosition===="+currentPausePositionInSong);
    ...
    private DialogInterface.OnClickListener click3=new DialogInterface.OnClickListener()
    {
        ...
        public void onClick(DialogInterface arg0,int arg1)
        {
            ...
            re = lovesong(loveid,ldDatas.get(currnetlistPosition).getSongid(),timestamp,cookie);        //删除
            if(re == "成功删除"){
                ldDatas.clear();                     //清空数据
                adapter.notifyDataSetChanged();     //通知适配器更新，以待重新载入数据
                loadsongData(listid);      //开始载入数据
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    URL url = new URL("http://192.168.1.108:3000/playlist/tracks?"+"op=add&pid="+listid+"&tracks=" + songid );        //本地IP经常变动，记得每次测试前先看看IP
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    HttpURLConnection connection;
                    //URL url = new URL("http://api.5288z.com/weixin/musicapi.php?q="+finalTitle);
                    URL url = new URL("http://192.168.1.108:3000/playlist/tracks?"+"op=del&pid="+listid+"&tracks=" + songid );        //本地IP经常变动，记得每次测试前先看看IP

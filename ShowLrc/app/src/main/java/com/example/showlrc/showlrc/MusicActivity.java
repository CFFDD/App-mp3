...
/*                    int m =mDatas.indexOf(mDatas.get(position));        //得到当前播放的音乐在LIST中的位置
    ...
    private void check(){
        ...
        Log.i(TAG," path===="+mDatas.get(position).path);
        Log.i(TAG," path===="+mDatas.get(position).path.length());

        while (mDatas.get(position).path.equals("null")){
            Toast.makeText(MusicActivity.this, "这首歌暂时没有音乐源,去听听下一首吧...", Toast.LENGTH_LONG).show();
            Log.i(TAG," 这首歌暂时没有音乐源,去听听下一首吧...");
            position++;
            mDatas.get(position).path =getAdress(mDatas.get(position).songid);
            mDatas.get(position).lyric =getLyric(mDatas.get(position).songid);
        }

        if(mDatas.get(position).lyric.equals("null")){
            Toast.makeText(MusicActivity.this, "这首歌暂时没有歌词...", Toast.LENGTH_LONG).show();
            Log.i(TAG," 这首歌暂时没有歌词...");
        }
    ...
    private void requestPermission() throws IOException {
        check();        //判断音乐源是否存在，无则加载或转道下一首      //没有音乐源大多是因为是VIP歌曲，需要会员权限

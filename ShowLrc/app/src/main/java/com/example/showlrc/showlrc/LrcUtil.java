...
public class LrcUtil {
    ...
    public static List<LrcBean> parseStr2List(String lrcStr) {
        ...
        //原本：跳过前四行，从第五行开始，因为前四行的歌词我们并不需要
        //now:从第一行开始
        for (int i = 0; i < subLrc.length; i++) {
            String lineLrc = subLrc[i];
            String ssss = lineLrc.substring(lineLrc.indexOf("[") + 1, lineLrc.indexOf(":") -1);
            if (isDigit1(ssss)==false) {//该段歌词格式不正确时将其丢弃
            }else{
                ...
                String mills = lineLrc.substring(lineLrc.indexOf(".") + 1, lineLrc.indexOf(".") + 2);

    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.getVercodeBtn:
                re =null;       //获取返回值之前先将re置为空，以防误判
                phone = phoneET.getText().toString().trim();
                Log.i(TAG, "phone.length===" + phone.length());
                if (phone == "") {
                    Toast.makeText(register.this, "请输入手机号码", Toast.LENGTH_LONG).show();
                } else if (isDigit1(phone) == false){      //验证手机号内容是否都为数字
                    Toast.makeText(register.this, "手机号码不符合规范（它应该是纯数字）", Toast.LENGTH_LONG).show();
                }else if (phone.length() != 11){
                    Toast.makeText(register.this, "手机号码不符合规范(它应该有11位)", Toast.LENGTH_LONG).show();
                }else{
                    String restring = sentmessage(phone);
                    if(restring == null){
                        Toast.makeText(register.this, "连接服务器失败，该手机号码今日发送验证码超过五次", Toast.LENGTH_LONG).show();
                    }else {
                        Log.i(TAG, "restring===" + restring);
                        Log.i(TAG, "restring.length()===" + restring.length());
            ...
            case R.id.registerBtn:
                re =null;       //获取返回值之前先将re置为空，以防误判
                setString();
                if (pwd.equals(confirm) == true) {       //直接使用  pwd == confirm  会出现  密码与确认密码不一致  的错误
                    Log.i(TAG, "pwd.equals(confirm) ==" + pwd.equals(confirm) );
                    re = checkvercode(phone,vercode);       //先检查验证码
                    if(re == "验证码正确"){
                        re =null;       //获取返回值之前先将re置为空，以防误判
                        re = regi(username, pwd, phone, vercode);       //验证码正确->进行注册
                        Log.i(TAG, "restring===" + re);
                        if (re == null) {
                            Toast.makeText(register.this, "该昵称已被注册", Toast.LENGTH_LONG).show();
                        } else if(re =="成功注册"){
                        ...
                        Toast.makeText(register.this, "验证码错误或手机号不正确", Toast.LENGTH_LONG).show();
                    }
                } else {
